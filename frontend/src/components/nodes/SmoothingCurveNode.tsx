...
import { defaultSmoothingCurveData, type SmoothingCurveNodeData } from "./smoothingCurveDefaults";

function patchSmoothingCurveData(
  id: string,
  patch: Partial<SmoothingCurveNodeData>,
  setNodes: (updater: (nodes: Node[]) => Node[]) => void,
) {
  setNodes((nodes) =>
    nodes.map((n) => {
      if (n.id !== id) return n;
      const cur = defaultSmoothingCurveData((n.data ?? {}) as Partial<SmoothingCurveNodeData>);
...
function smooth1dGaussian(values: number[], sigma: number): number[] {
  ...
  for (let i = 0; i < values.length; i += 1) {
    ...
    for (let j = -radius; j <= radius; j += 1) {
      const srcIdx = Math.max(0, Math.min(values.length - 1, i + j));
      const w = kernel[j + radius]! * inv;
      acc += values[srcIdx]! * w;
    }
    out[i] = acc;
  }
...
function ProcessedCurveChart({
  ...
  const clipId = `smooth-curve-clip-${Math.random().toString(36).slice(2, 9)}`;
...
export function SmoothingCurveNode({ id, data, selected }: NodeProps) {
  const d = defaultSmoothingCurveData(data as Partial<SmoothingCurveNodeData>);
  ...
  const update = useCallback(
    (patch: Partial<SmoothingCurveNodeData>) => patchSmoothingCurveData(id, patch, setNodes),
    [id, setNodes],
  );
  ...
  const compute = useCallback(async () => {
    ...
    const sigmaInSamples = Math.max(0.05, d.sigma / Math.max(1e-9, stepSpacing));
    const outValues = smooth1dGaussian(hydrated.values, sigmaInSamples);
    const bad = outValues.some((v) => !Number.isFinite(v));
    update({
      outputTensor: bad ? null : { shape: [...hydrated.shape], values: outValues },
      lastError: bad ? "Smoothed output contains non-finite values." : null,
    });
  }, [d.sigma, resolved, stepSpacing, update]);
    ...
    >
      <div className="cr-node__header cr-node__header--trainer">
        <div className="cr-node__header--row cr-node__header--trainer-main">
          <span className="cr-node__header-title">{readInstanceTitle(data as Record<string, unknown>, "Smoothing curve")}</span>
      ...
      <div className="cr-node__body">
        ...
        <ComfyFloatField
          ...
          min={0.05}
          max={128}
          title="Gaussian kernel sigma for smoothing; larger means smoother."
          onCommit={(n) => update({ sigma: Math.max(0.05, n) })}
          ariaLabel="Gaussian smoothing sigma"
        />
        ...
        <p className="cr-node__hint">Applies 1D Gaussian smoothing and outputs a same-length tensor.</p>
        <ProcessedCurveChart
          values={outputValues}
          stepTicks={stepTicks}
