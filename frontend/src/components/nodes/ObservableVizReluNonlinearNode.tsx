...
import {
  defaultObservableVizReluNonlinearData,
  type ObservableVizReluNonlinearNodeData,
} from "./observableVizReluNonlinearDefaults";
...
function patchData(
  id: string,
  patch: Partial<ObservableVizReluNonlinearNodeData>,
  setNodes: (updater: (nodes: Node[]) => Node[]) => void,
) {
  setNodes((nodes) =>
    nodes.map((n) => {
      if (n.id !== id) return n;
      const def = defaultObservableVizReluNonlinearData();
      const cur = (n.data ?? {}) as Partial<ObservableVizReluNonlinearNodeData>;
      const prev: ObservableVizReluNonlinearNodeData = {
...
export function ObservableVizReluNonlinearNode({ id, data, selected }: NodeProps) {
  const def = defaultObservableVizReluNonlinearData();
  const raw = (data ?? {}) as Partial<ObservableVizReluNonlinearNodeData>;
  const d: ObservableVizReluNonlinearNodeData = {
  ...
  const update = (patch: Partial<ObservableVizReluNonlinearNodeData>) => patchData(id, patch, setNodes);
  ...
  const clipId = `${id}-orn-clip`;

  const hint = isLive
    ? "Nonlinear ReLU count from last training run"
    : "No data yet — connect Trainer “observable”, then Train.";

  return (
    <div
      className={`cr-node cr-node--observable-viz-relu-nonlinear${selected ? " cr-node--selected" : ""}`}
      ...
      <div className="cr-node__body cr-node__body--tviz">
        ...
        <div className="cr-tviz-chart-wrap">
          <svg
            ...
            width={CHART_W}
            height={CHART_H}
            aria-label="Nonlinear ReLU neurons over training"
            onMouseDown={(e) => {
              if (!hasSeries) return;
