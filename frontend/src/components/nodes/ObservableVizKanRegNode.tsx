...
import {
  defaultObservableVizKanRegData,
  type ObservableVizKanRegNodeData,
} from "./observableVizKanRegDefaults";
...
function patchData(
  id: string,
  patch: Partial<ObservableVizKanRegNodeData>,
  setNodes: (updater: (nodes: Node[]) => Node[]) => void,
) {
  setNodes((nodes) =>
    nodes.map((n) => {
      if (n.id !== id) return n;
      const def = defaultObservableVizKanRegData();
      const cur = (n.data ?? {}) as Partial<ObservableVizKanRegNodeData>;
      const prev: ObservableVizKanRegNodeData = {
...
export function ObservableVizKanRegNode({ id, data, selected }: NodeProps) {
  const def = defaultObservableVizKanRegData();
  const raw = (data ?? {}) as Partial<ObservableVizKanRegNodeData>;
  const d: ObservableVizKanRegNodeData = {
  ...
  const update = (patch: Partial<ObservableVizKanRegNodeData>) => patchData(id, patch, setNodes);
  ...
  const clipId = `${id}-okr-clip`;

  const hint = isLive
    ? "KAN regularization term from last training run"
    : "No data yet — connect Trainer “observable”, then Train.";

  return (
    <div
      className={`cr-node cr-node--observable-viz-kan-reg${selected ? " cr-node--selected" : ""}`}
      ...
      <div className="cr-node__body cr-node__body--tviz">
        ...
        <div className="cr-tviz-chart-wrap">
          <svg
            ...
            width={CHART_W}
            height={CHART_H}
            aria-label="KAN regularization over training"
            onMouseDown={(e) => {
              if (!hasSeries) return;
