import type { MlpActivationId, MlpModelNodeData } from "../../components/nodes/mlpModelDefaults";
...
const KNOWN_KEYS = new Set(["inputDim", "outputDim", "depth", "width", "activation", "outputScale", "seed"]);

export const DEFAULT_MLP_SPEC_NAME = "MlpModel";

export const DEFAULT_MLP_PARAM_ORDER: (keyof MlpModelNodeData)[] = [
  "inputDim",
  "outputDim",
  "depth",
  "width",
...
function pyTypeForKey(key: keyof MlpModelNodeData): string {
  if (key === "activation") return "str";
  if (key === "outputScale") return "float";
  return "int";
}
...
function formatPyDefault(key: keyof MlpModelNodeData, v: unknown): string {
...
export function generateMlpModelSpecCode(d: MlpModelNodeData, order: string[], specName: string): string {
  const name = specName.trim() || DEFAULT_MLP_SPEC_NAME;
  const lines: string[] = [`import torch`, ``, `class ${name}(torch.nn.Module):`, `    def __init__(`, `        self,`];
  const merged = { ...defaultMlpModelData(), ...d };
  const keys = (order.length ? order : DEFAULT_MLP_PARAM_ORDER)
    .filter((k) => KNOWN_KEYS.has(k))
    // outputScale is mlp_model-only; node types that never carried it
    // this generator via buildMlpTorch) must not grow an output_scale param.
    .filter((k) => k !== "outputScale" || "outputScale" in d);
  for (const k of keys) {
    const ck = k as keyof MlpModelNodeData;
  ...
  lines.push(`            layers.append(acts.get(str(activation), torch.nn.ReLU)())`);
  lines.push(`            in_f = int(width)`);
  lines.push(`        layers.append(torch.nn.Linear(in_f, int(output_dim)))`);
  lines.push(`        self.net = torch.nn.Sequential(*layers)`);
  lines.push(``);
...
export function parseMlpClassInitHeader(code: string): { className: string; paramsChunk: string; error?: string } {
...
export function parseMlpModelSpecCode(code: string): {
  specName: string;
  paramOrder: string[];
  ...
  for (const row of params) {
    ...
    const key = camel as keyof MlpModelNodeData;
    if (key === "activation") {
