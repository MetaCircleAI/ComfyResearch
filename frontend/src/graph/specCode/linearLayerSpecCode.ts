...
export const DEFAULT_LINEAR_LAYER_SPEC_NAME = "LinearLayer";

export const DEFAULT_LINEAR_LAYER_PARAM_ORDER: (keyof LinearLayerNodeData)[] = [
  "inFeatures",
  "outFeatures",
  "bias",
  "seed",
...
function formatPyDefault(key: keyof LinearLayerNodeData, v: unknown): string {
...
function pyTypeForKey(key: keyof LinearLayerNodeData): string {
  if (key === "bias") return "bool";
  return "int";
}

export function generateLinearLayerSpecCode(
  d: LinearLayerNodeData,
  order: string[],
  specName: string,
): string {
  const name = specName.trim() || DEFAULT_LINEAR_LAYER_SPEC_NAME;
  const lines: string[] = [`import torch`, ``, `class ${name}(torch.nn.Module):`, `    def __init__(`, `        self,`];
  const merged = { ...defaultLinearLayerData(), ...d };
  const keys = (order.length ? order : DEFAULT_LINEAR_LAYER_PARAM_ORDER).filter((k) => KNOWN_KEYS.has(k));
  for (const k of keys) {
    const ck = k as keyof LinearLayerNodeData;
    const sn = camelToSnakeCase(String(ck));
    lines.push(`        ${sn}: ${pyTypeForKey(ck)} = ${formatPyDefault(ck, merged[ck])},`);
  }
  lines.push(`    ):`);
  lines.push(`        super().__init__()`);
  lines.push(`        self.lin = torch.nn.Linear(int(in_features), int(out_features), bias=bias)`);
  lines.push(``);
  lines.push(`    def forward(self, x: torch.Tensor) -> torch.Tensor:`);
  lines.push(`        return self.lin(x)`);
  return lines.join("\n");
}

function parseLinearClassInitHeader(code: string): { className: string; paramsChunk: string; error?: string } {
...
export function parseLinearLayerSpecCode(code: string): {
  specName: string;
  paramOrder: string[];
  ...
  const cls = parseLinearClassInitHeader(code);
  if (!cls.error) {
  ...
  const patch: Partial<LinearLayerNodeData> = {};
  ...
  for (const row of params) {
    ...
    const key = camel as keyof LinearLayerNodeData;
    if (key === "bias") {
