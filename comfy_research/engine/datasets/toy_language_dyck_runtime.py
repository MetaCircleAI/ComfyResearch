"""Well-formed Dyck (balanced bracket) sequences mapped to token ids."""
...
from comfy_research.engine.datasets.toy_language_common import dataset_rng_seed, scalar_int, slice_shifted_window_lm
...
def _sample_dyck_row(
    ...
    while len(seq) < target_len:
        # When ``v == 2*k`` there is no token id strictly above the bracket alphabet, so
        # ``[filler_lo, v)`` can be a singleton ``{v-1}`` and every padded tail would be ``v-1``.
        # Fall back to uniform draws on ``[0, v)`` whenever that range would otherwise collapse.
        lo, hi = filler_lo, v
        if lo >= hi - 1:
            lo = 0
...
def build_dyck_lm_arrays(
    ...
    # Vocabulary is exactly 2k token ids: k open brackets at 0,2,... and k closes at 1,3,...
    v = 2 * k_bracket
    k_eff = k_bracket
    target_len = ctx + 1
