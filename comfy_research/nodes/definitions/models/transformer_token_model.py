"""transformer_token_model — ModelDef-channel definition + builder provider.
...
DEF = model_def(
    ModelDef(
        ...
        fields=(
            ...
            EnumField(key="tieEmbeddingLmHead", label="Tie Embedding Lm Head", default='yes', options=('yes', 'no')),
        ...
        frontend=FrontendSpec(component_key="TransformerTokenModelNode", codegen_key="transformer_token_model"),
    )
)
...
def build(data, context):
    from comfy_research.engine.models.model_builders import _build_transformer_token_model

    return _build_transformer_token_model(data, context)
